...
/**
 * A buffering jdbc sink. Expects properties contains the following
 * settings:
 ...
 *   - `buffer.size` the number of elements to buffer before writing out to
 *     jdbc connection
 *
 * @param props
 *   the properties used to configure the sink
 * @tparam E
...
class JdbcSink[E: TypeInformation](
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    if (
      connection.isClosed
    ) {} // force usage of this to avoid never used warning
  ...
  override def initializeState(
      context: FunctionInitializationContext): Unit = {
