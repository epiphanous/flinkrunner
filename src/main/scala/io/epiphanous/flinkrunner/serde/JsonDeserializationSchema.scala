...
class JsonDeserializationSchema[
    ...
    ADT <: FlinkEvent](sourceConfig: SourceConfig[ADT])
  ...
  /** Deserialize a json byte array into an event instance or return null
    * if the byte array can't be successfully deserialized
    * @param bytes
  ...
  /** Determine if the next event is the end of the stream or not. We
    * always return false since we assume the stream never ends.
    * @param nextEvent
    *   the next event
    * @return
    *   false
