...
class AvroCoder(registry: AvroSchemaRegistryClient) {
  ...
  protected def getByteBuffer(message: Array[Byte]): Try[ByteBuffer] =
    Try(ByteBuffer.wrap(message)).flatMap { buffer =>
      Try(buffer.get()).flatMap[ByteBuffer] { b =>
        ...
        else Failure(new AvroCodingException(s"unexpected magic byte '$b' found while decoding message"))
      }
    }
  ...
  /**
    * Avro encode an event record, using the latest schema found in the registry under the event record's
    * class name (as returned by event.getClass.getSimpleName).
    *
    * @param event E An event record
...
object AvroCoder {
  ...
  /**
    * A magic byte that should be the first byte read from a byte buffer amenable to encoding with this library.
    */
