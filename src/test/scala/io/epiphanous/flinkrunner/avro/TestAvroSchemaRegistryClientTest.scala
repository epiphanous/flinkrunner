...
class TestAvroSchemaRegistryClientTest
    extends AnyFlatSpec
    with Matchers
    with TryValues
    with LazyLogging {
  ...
  it should "subject for key" in {
    registry.getSubjectName(
      testObj,
      Some(ConfluentSchemaRegistryContext(isKey = true))
    ) shouldEqual "io.epiphanous.flinkrunner.avro.TestAvroClass1-key"
  }

  it should "subject for value" in {
    registry.getSubjectName(
      testObj
    ) shouldEqual "io.epiphanous.flinkrunner.avro.TestAvroClass1-value"
  }

  it should "install and get" in {
    def runScenario(isKey: Boolean): Unit = {
      val subject      = registry.getSubjectName(
        testObj,
        Some(ConfluentSchemaRegistryContext(isKey = isKey))
      )
      val regSchema    =
      ...
      val getByObj     = registry.getFromEvent(testObj)
      getByObj.success.value._1 shouldEqual regSchema
      registry.clear()
    }
