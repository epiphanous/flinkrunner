...
import io.epiphanous.flinkrunner.model.{KinesisProperties, MySimpleADT}
...
class FirehoseSinkConfigSpec extends PropSpec {
  ...
  property("unconfigured firehose") {
    ...
    the[RuntimeException] thrownBy runner
      .getSinkConfig() should have message "kinesis stream name required but missing in sink <firehose-sink> of job <firehose>"
  }
